...
public class FpsUtils {
    ...
    //监听时的刷新时间间隔,设置为1S
    public static final long FPS_INTERVAL_TIME = 1000;
    //低帧判断  fps<25判断为低帧
    ...
    public void addFrame(){
        ...
        this.count++;
        this.totalCount++;
        //Log.e("TWT", "addFrame: "+(System.currentTimeMillis() - lastTime) );
        //Log.d("TWT", "Time: "+time);
        //判断是否为低帧
        if(time>LOW_FRAME_TIME){
            lowCount++;//Log.e("TWT", "Time: "+ Last3FrameTimes.get(2));
        }
        //判断是否为jank帧
        if(isJank(time)){
            //Log.e("TWT", "JANK: "+(System.currentTimeMillis()-lastTime));
            jankCount++;
        }
        //更新渲染帧时间戳
        this.lastTime = System.currentTimeMillis();
    ...
    public boolean isJank(int time){
        ...
        //Last3FrameTimes.size()
        if(size<3){
            Last3FrameTimes.add(time);
//            if(time>LOW_FRAME_TIME*2){
//                return true;
//            }else{
                return false;
//            }
